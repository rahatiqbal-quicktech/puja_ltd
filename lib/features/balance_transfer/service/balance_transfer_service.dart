...
class BalanceTransferService {
  sendBalanceTransferRequest({
    required String receiverMember,
    required String amount,
  }) async {
    EasyLoading.show();
    var box = GetStorage();
    try {
      String endPoint = "/api/v1/balance/transfer";
      String url = baseUrl + endPoint;

      Map<String, dynamic> data = {
        "receiver_member": receiverMember,
        "amount": amount
      };
      ...
      if (response.statusCode == 200) {
        EasyLoading.dismiss();
        Get.back();
        Fluttertoast.showToast(msg: "Balance Transfer request sent");
      } else {
        EasyLoading.dismiss();
